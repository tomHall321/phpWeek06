...
class Validator
{
    public function email($possibleEmail)
    {
        $removeSpaces = preg_replace("/(^\s+)|(\s+$)/", "", $possibleEmail);
        return filter_var($removeSpaces, FILTER_VALIDATE_EMAIL) === $removeSpaces;
    }

    public function postcode($possiblePostcode)
    {   
        return preg_match("/^[A-Z][A-Z]?[0-9][0-9]?[0-9A-Z]? [0-9][A-Z][A-Z]$/", $possiblePostcode) === 1;    
    }
}
